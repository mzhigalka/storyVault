...
export default function Home() {
  ...
  const { data, isLoading, isError } = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await fetch(`/api/stories?sort=${sort}&page=${currentPage}&limit=${itemsPerPage}`);
  ...
  const sortedStories = sortBy === "expiring" 
    ? [...stories].sort((a, b) => new Date(a.expiresAt).getTime() - new Date(b.expiresAt).getTime())
    : stories;
    ...
    <>
      ...
      <section className="mb-8 bg-gradient-to-r from-primary/90 to-primary rounded-lg shadow-lg overflow-hidden">
        <div className="px-6 py-12 md:py-20 max-w-3xl mx-auto text-center">
          ...
          <p className="text-white/90 mb-8 text-lg">Create time-limited stories, share them with your friends, and let the world vote on your creativity.</p>
          <div className="flex flex-col sm:flex-row justify-center gap-4">
            <Button
              size="lg"
              className="bg-white text-primary hover:bg-gray-100"
              onClick={() => document.getElementById("create-story-button")?.click()}
            >
              Create a Story
            </Button>
            <Button
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
          {Array(6).fill(0).map((_, i) => (
            <div key={i} className="bg-white rounded-lg shadow-sm overflow-hidden border border-light">
              <div className="p-5">
        ...
        <div className="text-center py-12">
          <p className="text-lg text-muted">Failed to load stories. Please try again later.</p>
