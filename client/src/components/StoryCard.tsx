...
export default function StoryCard({
  ...
  const { mutate: voteForStory, isPending: isVoting } = useMutation({
    mutationFn: async ({
      ...
      console.log(
        `${action === "vote" ? "Voting for" : "Removing vote from"} story ID:`,
        storyIdStr
      );
      console.log("Story object:", story);
    ...
    onError: (error: any) => {
      ...
      toast({
        ...
        description:
          error.message ||
          "Не вдалося виконати операцію з голосуванням. Спробуйте пізніше.",
        variant: "destructive",
      });
    },
  });
  ...
  const handleVote = () => {
    ...
    voteForStory({ storyId: story._id || story.id, action });
  };
            ...
            {isHome && (
              <>
                <Button
                  variant="ghost"
                  size="sm"
                  className={`text-muted-all p-0 transition-colors ${
                    hasVoted ? "text-primary" : "hover:text-primary"
                  } ${isVoting ? "opacity-50 cursor-not-allowed" : ""}`}
                  onClick={handleVote}
                  disabled={isVoting}
                  title={hasVoted ? "Забрати голос" : "Проголосувати"}
                >
                  {renderVoteIcon()}
                </Button>
                <span className="font-medium text-dark">{story.votes}</span>
