...
export default function StoryCard({
  ...
  const handleVote = () => {
    ...
    if (localHasVoted) {
      toast({
        title: "Уже проголосовано",
        description: "Вы уже проголосовали за эту историю.",
        variant: "default",
      });
      return;
    }
            ...
            {isHome && (
              <>
                <Button
                  variant="ghost"
                  size="sm"
                  className={`text-muted-all p-0 transition-all duration-200 ${
                    localHasVoted ? "text-primary" : "hover:text-primary"
                  } ${isVoting ? "opacity-50 cursor-not-allowed" : ""}`}
                  onClick={handleVote}
                  disabled={isVoting || localHasVoted}
                >
                  {localHasVoted ? (
                    <ArrowUpCircle className="h-5 w-5 fill-current" />
                  ) : (
                    <ArrowUp className="h-5 w-5" />
                  )}
                </Button>
